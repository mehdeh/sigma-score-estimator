"""
ResNet model for estimating omega(x) - noise-level score gradient.
This model takes only the image as input and outputs a single scalar.
"""
...
class ResNetOmegaX(nn.Module):
    """
    ResNet18-based model for estimating omega(x).
    
    This model estimates the noise-level score gradient for a given noisy image,
    without explicit noise level conditioning.
    ...
    Output:
        Scalar tensor of shape (batch_size, 1) representing omega(x)
    """
    
    def __init__(self, block=BasicBlock, num_blocks=[2, 2, 2, 2]):
        super(ResNetOmegaX, self).__init__()
        self.in_planes = 64
...
def resnet18_omega_x():
    """
    Create a ResNet18-based model for omega(x) estimation.
    
    Returns:
        ResNetOmegaX model instance
    """
    return ResNetOmegaX(BasicBlock, [2, 2, 2, 2])
